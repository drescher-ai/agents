...
class _ImplOptions:
    ...
    def set_allow_interruptions(self, allow_interruptions: bool) -> _ImplOptions:
        object.__setattr__(self, 'allow_interruptions', allow_interruptions)
...
class VoicePipelineAgent(utils.EventEmitter[EventTypes]):
    ...
    def __init__(
        ...
        *,
        ...
        if hasattr(self._llm, "set_voice_pipeline_agent"):
            self._llm.set_voice_pipeline_agent(self)
        ...
        # ----- NOVA FEATURE: Canal especial para mensagens proativas do Bot -----
        self._interrupt_message_queue: asyncio.Queue[Union[str, LLMStream, AsyncIterable[str]]] = asyncio.Queue()
        self._interrupt_message_task: Optional[asyncio.Task] = None
        # ---------------------------------------------------------------------------
        self._current_interrupt_id: Optional[int] = None
        self._current_interrupt_queue: Optional[asyncio.Queue[str]] = None
        self._current_interrupt_handle: Optional[SpeechHandle] = None
    ...
    def start(
        ...
        self._interrupt_message_task = asyncio.create_task(self._interrupt_message_listener())
    ...
    # def _change_opt(self, opt_name: str, value: Any) -> None:
    #     if not hasattr(self._opts, opt_name):
    #         raise ValueError(f"Unknown option: {opt_name}")
    #     elif getattr(self._opts, opt_name) == value:
    #         return
    #     else:
    #         setattr(self._opts, opt_name, value)

    def set_interruption_allowed(self, allow_interruptions: bool) -> None:
        """Set whether interruptions are allowed""" 
        self._opts.set_allow_interruptions(allow_interruptions)
        logger.info(f"Interruptions are now {'allowed' if allow_interruptions else 'disallowed'}")
        logger.info(self._opts)
    ...
    # ------------- NOVA FUNÇÃO: Canal especial para mensagens proativas do Bot --------------
    async def _interrupt_message_listener(self) -> None:
        """
        Esta task fica aguardando por chunks de mensagem proativa do Bot.
        Cada chunk é um dicionário no seguinte formato:
            {
        ...
        Se o interrupt_id do chunk recebido for maior do que o em curso (ou se nenhum estiver ativo),
        o método cancela sínteses em andamento, limpa a fila de speeches e cria um novo SpeechHandle.
        Caso o interrupt_id seja igual ao atual, o chunk pertence à mesma mensagem – neste caso,
        o conteúdo é encaminhado via uma fila associada ao stream já criado, sem reiniciar a síntese.
        Quando 'content_complete' é True, o stream é finalizado.
        O flag no_interruption_allowed é utilizado para definir o parâmetro allow_interruptions do SpeechHandle.
        """
        logger.info("Iniciando listener de mensagens proativas do Bot")
        while True:
            chunk = await self._interrupt_message_queue.get()
            if chunk.get("response_type") != "agent_interrupt":
            ...
            if self._current_interrupt_id is None or chunk_interrupt_id > self._current_interrupt_id:
                ...
                # Cria uma nova fila para encaminhamento imediato dos chunks desta nova mensagem
                new_queue = asyncio.Queue()
                # Encaminha o primeiro chunk imediatamente
                new_queue.put_nowait(chunk.get("content", ""))
                # Define um gerador assíncrono que repassa os conteúdos conforme chegam na fila
                async def proactive_stream_gen(q: asyncio.Queue) -> AsyncGenerator[str, None]:
                    while True:
                        data = await q.get()
                        if data is None:
                            break
                ...
                self._current_interrupt_handle = proactive_handle

            # Chunk pertencente à mesma interrupção em curso
            elif chunk_interrupt_id == self._current_interrupt_id:
                if self._current_interrupt_queue is not None:
                    await self._current_interrupt_queue.put(chunk.get("content", ""))

            # Se o chunk indica que a mensagem está completa, sinaliza o fim do stream
            if chunk.get("content_complete"):
                if self._current_interrupt_queue is not None:
                    await self._current_interrupt_queue.put(None)
                # Reseta o estado da interrupção para permitir novos interrupts
                # self._current_interrupt_id = None
                self._current_interrupt_queue = None
                self._current_interrupt_handle = None

            else:
                # Caso o chunk tenha um interrupt_id menor que o atual, ignora-o.
                pass

    async def send_proactive_message(self, source: str | LLMStream | AsyncIterable[str]) -> None:
        """
        Método público para enviar uma mensagem proativa do Bot.
        Ao chamar este método, a mensagem é colocada na fila do canal especial.
        """
        await self._interrupt_message_queue.put(source)
    # -----------------------------------------------------------------------------------------
